...
export class PatrolScene extends Phaser.Scene {
  ...
  create() {
    ...
    const ensureAnim = (animation: string, direction: DirectionId) => {
      ...
      const resolved: ResolvedSpriteSheetSpec = resolveSheetDirection(resolvedSheet, direction)
      this.anims.create({
        key: animKey,
    ...
    this.sprite = this.add.sprite(startX, y, `${sheet.model}:${sheet.animation}`, initialResolved.startFrame)
  ...
  update(_: number, deltaMs: number) {
    ...
    if (this.turningMsLeft > 0) {
      ...
      if (this.turningMsLeft === 0) {
        ...
        const animKey = this.ensureAnimFor(sheet.model, sheet.animation, direction)
  ...
  private onMove(dir: unknown) {
    ...
    const nextAnimation = d === 0 ? baseAnimation : getAnimationUrl(this.cfg.sheet.model, 'walk') ? 'walk' : baseAnimation
  ...
  private ensureAnimFor(model: string, animation: string, direction: DirectionId) {
